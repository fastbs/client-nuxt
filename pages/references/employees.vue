        ...
        <pDataTable :value="tableData" dataKey="id" editMode="cell" @cell-edit-complete="onCellEditComplete($event)"
            ...
            <template #header>
                <div class="flex justify-content-between">
                    <div>
                        <pButton v-if="UsersService.checkPermission('employees', 'create')" label="Добавить"
                            icon="pi pi-plus" severity="success" class="mr-2" :disabled="disableCreate" @click="addData" />
                        <pButton v-if="UsersService.checkPermission('employees', 'delete')" label="Удалить"
                            icon="pi pi-trash" severity="danger" :disabled="!selected || !selected.length || disableDelete"
                            @click="deleteSelected()" class="mr-4" />
                        <pButton
                            v-if="UsersService.checkPermission('employees', 'create') || UsersService.checkPermission('employees', 'delete') || UsersService.checkPermission('employees', 'update')"
            ...
            <pColumn v-for="col of tableColumns" :field="col.field" :hidden="col.hidden" :header="col.header"
                ...
                <template v-if="UsersService.checkPermission('employees', 'update') && col.field !== 'user_created'"
                    #editor="{ data, field }">
...
import type { EmployeeDto, CreateEmployeeDto, UpdateEmployeeDto } from "@/services/dto/employees.dto";
...
const getData = async () => {
    ...
    const res_jobtitles = await EmployeesService.fetch();
    tableData.value = [];
    selected.value = [];
...
const onCellEditComplete = (event: DataTableCellEditCompleteEvent) => {
    ...
    if (value != newValue) {
        if (d && d <= lastId && !updatedData.includes(d)) {
            updatedData.push(d);
            console.log("updatedData:", updatedData, updatedData.includes(d));
        }
...
const saveData = async () => {
    if (deletedData.length) {
        await EmployeesService.deleteMany(deletedData);
    }

    for (const id of updatedData) {
        const item = tableData.value.find(x => x.id == id);
        if (item) {
            const ujt: UpdateEmployeeDto = {
                name: item.name,
                name_short: item.name_short,
                company: item.company.id,
            };
            await EmployeesService.update(id, ujt);
        }
    }

    const newData = tableData.value?.filter(x => x.id > lastId);
    for (const item of newData) {
        const ce: CreateEmployeeDto = {
            name: item.name,
            name_short: item.name_short,
            company: item.company.id,
        }
        await EmployeesService.create(ce);
    }
